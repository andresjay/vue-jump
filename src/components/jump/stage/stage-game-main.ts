...
export default class StageGameMain {
  ...
  onTouchEnd = (e) => {
    ...
    this.hit = this.getHitStatus(
      ...
      this.currentBlock,
      this.nextBlock,
      BLOCKCONFIG.height / 2 - (1 - this.currentBlock.instance.scale.y) * BLOCKCONFIG.height
    )
  ...
  setDirection(direction) {
    ...
    this.axis = new THREE.Vector3(
      ...
      0,
      this.targetPosition.z - currentPosition.z
    )
    this.axis.normalize()
  ...
  checkBottleHit() {
    if (
      this.checkingHit &&
      this.bottle.instance.position.y <= BLOCKCONFIG.height / 2 + 0.1 &&
  ...
  getHitStatus(bottle, currentBlock, nextBlock, initY) {
    ...
    if (nextBlock) {
      ...
      if (utils.pointInPolygon(destination, nextPolygon)) {
        ...
        utils.pointInPolygon([destination[0] - this.bottle.width, destination[1]], nextPolygon) ||
        utils.pointInPolygon([destination[0], destination[1] + this.bottle.depth / 2], nextPolygon)
      ) {
        result1 = GAME_OVER_NEXT_BLOCK_BACK
      } else if (
        utils.pointInPolygon(
          [destination[0], destination[1] - this.bottle.depth / 2],
          nextPolygon
        ) ||
        utils.pointInPolygon([destination[0] + this.bottle.depth / 2, destination[1]], nextPolygon)
      ) {
    ...
    if (utils.pointInPolygon(destination, currentPolygon)) {
      ...
      utils.pointInPolygon([destination[0], destination[1] + this.bottle.depth], currentPolygon) ||
      utils.pointInPolygon([destination[0] - this.bottle.width / 2, destination[1]], currentPolygon)
    ) {
  ...
  updateNextBlock() {
    ...
    if (type === BLOCKTYPE.CUBOID) {
      this.nextBlock = new CuboidBlock(
        targetPosition.x,
        targetPosition.y,
        targetPosition.z,
        'popup',
      ...
      this.nextBlock = new CylinderBlock(
        targetPosition.x,
        targetPosition.y,
        targetPosition.z,
        'popup',
