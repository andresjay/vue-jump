...
import img_top from '@/static/images/top.png'
...
class Bottle {
  ...
  _jump(tickTime) {
    ...
    const translateY =
      this.velocity.vy * t - 0.5 * this.gravity * t * t - this.gravity * this.flyingTime * t
  ...
  forerake() {
    ...
    setTimeout(() => {
      if (this.direction === 0) {
        customAnimation.to(this.instance.rotation, 0.5, { z: -Math.PI / 2 })
      }
      if (this.direction === 1) {
        customAnimation.to(this.instance.rotation, 0.5, { x: -Math.PI / 2 })
      }
      setTimeout(() => {
